...
public class ImageUtil { 
	 ...
	 public String compressPic() { 
		 try { 
			 ...
			 if (img.getWidth(null) == -1) {
				 ...
				 System.out.println("don't need to compress!");
				 return "no";
			 } else { 
 	...
 	public static void main(String[] arg) { 
 		String saveFileName ="d:\\picture\\";
 		String inputFileName = "1寸照海涛.jpg";
 		String outputFileName = "---1";
 		
 		ImageUtil mypic = new ImageUtil(); 
 		System.out.println("输入的图片大小：" + mypic.getPicSize(saveFileName+inputFileName)/1024 + "KB"); 
 		int count = 0; // 记录全部图片压缩所用时间
 		for (int i = 0; i < 40; i++) { 
 			int start = (int) System.currentTimeMillis();	// 开始时间 
 			System.out.println(mypic.compressPic(saveFileName,saveFileName, inputFileName, outputFileName+i+".jpg", 500, 500, true)); 
 			int end = (int) System.currentTimeMillis(); // 结束时间 
 			int re = end-start; // 但图片生成处理时间 
 			count += re; System.out.println("第" + (i+1) + "张图片压缩处理使用了: " + re + "毫秒"); 
 			System.out.println("输出的图片大小：" + mypic.getPicSize(saveFileName+outputFileName+i+".jpg")/1024 + "KB"); 
 		}
