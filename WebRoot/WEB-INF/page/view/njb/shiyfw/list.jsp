	...
	<div class="nchjbxx-mainbg box">
		...
		<div class="nchjbxx-mainbgright">
			<div class="nchjbxx-main">
				...
				<div class="sitebg">
					<div class="site">
						<b>当前位置：共青团农场社区管理平台&nbsp;&gt;&gt;&nbsp;人员管理&nbsp;&gt;&gt;&nbsp;住户管理&nbsp;&gt;&gt;&nbsp;实有房屋管理</b>
					</div>
				</div>
				<!--当前位置  结束-->
				...
				<div class="tdzy-tab-down">
					...
					<form action="" method="post" id="seach">
						<div class="chaxun-form">
							<b>查询:</b> <span> <label>小区名称</label> <input name="_page.params.name"
							...
							<tbody>
								<c:forEach items="${_page.data }" var="model" varStatus="i">
									<tr <c:if test="${i.count%2 == 0 }">class="ancolorbg"</c:if>>
										...
										<td>${model.xqdz }</td>
										<td>${model.zhuang}</td>
										<td><c:if test="${!fn:contains(model.fwlx,'平房') }">${model.dy}</c:if></td>
										<td><c:if test="${!fn:contains(model.fwlx,'平房') }">${model.shi}</c:if></td>
										<td>${model.cqr}</td>
										<td><fmt:formatDate value="${model.jfsj}" pattern="yyyy-MM-dd"/></td>
										<td>
