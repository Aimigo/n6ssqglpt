	...
	<div class="nchjbxx-mainbg box">
		...
		<div class="nchjbxx-mainbgright">
			<div class="nchjbxx-main">
				...
				<div class="sitebg">
					<div class="site">
						<b>当前位置：共青团农场社区管理平台&nbsp;&gt;&gt;&nbsp;民生事务&nbsp;&gt;&gt;&nbsp;目录管理&nbsp;&gt;&gt;&nbsp;修改</b>
					</div>
				</div>
				<!--当前位置  结束-->
				
				<!--表单内容  开始-->
				<form action="mlgl_update.action" method="post" name="updateform">
				<div class="biaodan-all">
					...
					<table class="biaodan" cellpadding="0" cellspacing="0">
						...
						<tr>
							...
							<td colspan="3">
								<input name="bt" type="text" class="qbg" maxlength="50" value="${mlgl.bt }"
									dataType="Require" msg="标题不能为空！" require="true"/>
								<input name="id" type="hidden" value="${mlgl.id }"/>
								<input name="scsj" type="hidden" value="${mlgl.scsj }"/>
								<input name="userid" type="hidden" value="${mlgl.userid }"/>
							</td>
						</tr>
						<tr>
							...
							<td colspan="3">
								<select name="flid" dataType="Require" msg="目录分类不能为空！" require="true">
									...
									<c:forEach items="${mlfllist }" var="mlfl">
										<option value="${mlfl.id }" <c:if test="${mlgl.flid eq mlfl.id }">selected="selected"</c:if>>${mlfl.name }</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td colspan="3">
								<textarea name="ms" id="ms" type="text" max="1000" style="width:98%;"
									dataType="Require" msg="描述不能为空！" require="true">${mlgl.ms }</textarea>
							</td>
						</tr>
						<tr>
							<td valign="top">上传附件（<font class="fred">*</font>）</td>
							<td colspan="3">
								<input name="dz" id="dz" type="hidden" class="qbg" maxlength="50" value="${mlgl.dz }"
