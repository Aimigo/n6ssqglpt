	...
	<div class="nchjbxx-mainbg box">
		...
		<div class="nchjbxx-mainbgright">
			<div class="nchjbxx-main">
				...
				<div class="sitebg">
					<div class="site">
						<b>当前位置：共青团农场社区管理平台&nbsp;&gt;&gt;&nbsp;就业服务&nbsp;&gt;&gt;&nbsp;技能培训多媒体&nbsp;&gt;&gt;&nbsp;多媒体管理&nbsp;&gt;&gt;&nbsp;修改</b>
					</div>
				</div>
				<!--当前位置  结束-->
				...
				<!--表单内容  开始-->
				<form action="pxjn_update.action" method="post" name="updateform">
				<div class="biaodan-all">
					...
					<table class="biaodan" cellpadding="0" cellspacing="0">
						...
						<tr>
							...
							<td colspan="3">
								<input name="bt" type="text" class="qbg" maxlength="50" value="${pxjn.bt }"
									dataType="Require" msg="标题不能为空！" require="true"/>
								<input name="id" type="hidden" value="${pxjn.id }"/>
								<input name="scsj" type="hidden" value="${pxjn.scsj }"/>
								<input name="userid" type="hidden" value="${pxjn.userid }"/>
							...
							<td colspan="3">
								<select name="flid" dataType="Require" msg="多媒体分类不能为空！" require="true">
									...
									<c:forEach items="${jnfllist }" var="jnfl">
										<option value="${jnfl.id }" <c:if test="${pxjn.flid eq jnfl.id }">selected="selected"</c:if>>${jnfl.name }</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr valign="top">
							<td>描述（<font class="fred">*</font>）</td>
							<td colspan="3">
								<textarea name="ms" id="ms" type="text" style="width:98%;"
									dataType="Require" msg="描述不能为空！" require="true">${pxjn.ms }</textarea>
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="3">
								<input name="dz" id="dz" type="hidden" class="qbg" maxlength="50" value="${pxjn.dz }"
						...
						<tr>
							...
							<td colspan="3">
								...
								<input type="hidden" id="tp" name="tp" value="${pxjn.tp }"/>
								<input type="hidden" id="upload_url0" value="" />
								<input type="hidden" id="upload_url1" value="" />
								<input type="hidden" id="upload_url2" value="" />
