...
<script type="text/javascript">
	function detail(id) {
		window.location.href = "ylzs_detail.action?id=" + id;
	}
	...
	function deleteIt(id) {
		if (confirm("您确定要删除吗？")) {
			$.ajax({
				type : "post",
				url : "ylzs_delete.action",
				data : {
					"id" : id
				},
				dataType : "html",
				success : function(data) {
					window.location.href = "ylzs_list.action";
				}
			});
		}
	...
	function show(id,type) {
		var url = "ylzs_add.action";
		if (id)
			url = "ylzs_edit.action?id=" + id;
		if(type)
			url = "ylzs_detail.action?id=" + id;
	...
	$(function() {
		...
		$("[name='_page.params.flid']").val(${_page.params.flid});
	});
</script>
</head>
	...
	<div class="nchjbxx-mainbg box">
		...
		<div class="nchjbxx-mainbgright">
			<div class="nchjbxx-main">
				...
				<div class="sitebg">
					<div class="site">
						<b>当前位置：共青团农场社区管理平台&nbsp;&gt;&gt;&nbsp;医疗卫生&nbsp;&gt;&gt;&nbsp;医疗知识&nbsp;&gt;&gt;&nbsp;医疗知识</b>
					</div>
				</div>
				<!--当前位置  结束-->
				...
				<div class="tdzy-tab-down">
					<!--查询  开始-->
					<form action="ylzs_list.action" method="post">
					<div class="chaxun-form">
						...
						<%--<span>
							<label>知识内容</label>
							<input name="_page.params.nr" type="text" value="${_page.params.nr }" class="bd-r" />
						</span>--%>
						<span>
							<label>知识分类</label>
							<select name="_page.params.flid" class="bd-r">
								...
								<c:forEach items="${ylzsfl }" var="zsfl">
									<option value="${zsfl.id }">${zsfl.name }</option>
								</c:forEach>
							</select>
						</span> 
						<span>
