		...
		<div class="nchjbxx-mainbgright">
			<div class="nchjbxx-main">
				...
				<!--选项卡下方边框  开始-->
					...
					<!--表格内容  开始-->
					<link rel="stylesheet" href="fun/system/css/link.css" type="text/css"/>
					<div class="pxkchvideo-border">
						<c:forEach items="${_page.data }" var="obj" varStatus="i">
							<ul class="jkzx">
							<c:choose>
							<c:when test="${obj.hfsj ne null and obj.hfsj ne '' }">
								<li class="jkzx_hd">
									<h2 class="jkzx_title">
										<span>${obj.flname } <fmt:formatDate value="${obj.zxsj }" pattern="yyyy-MM-dd HH:mm:ss" /></span>
										<div class="jkzx_icon">
											<c:if test='${fn:contains(operationlist,"修改") }'>
												<c:if test="${_page.params.type eq '1' and obj.hfsj eq null}">
													<em onclick="javascript:show(${obj.id});" title="编辑"></em>
												</c:if>
												<c:if
													test="${_page.params.type eq '2' and obj.hfsj eq null}">
													<em onclick="javascript:show(${obj.id});" title="咨询回复"></em>
												</c:if>
											</c:if>
											<c:if test='${fn:contains(operationlist,"删除") }'>
												<c:if test="${obj.hfsj eq null}">
												<i onclick="javascript:deleteIt(${obj.id});" title="删除"></i>
												</c:if>
											</c:if>
										</div>
									</h2>
									<div class="jkzx_main">
										<span>${obj.yhname }：${obj.zxbt}</span><br />
										<span>${obj.zjname }：${fn:substring(obj.zxhf,0,110) }</span><br />
							...
							<c:otherwise>
								<li class="jkzx_whd">
									...
									<div class="jkzx_main">
										<span>${obj.yhname }：${obj.zxbt}</span><br /> 
										<span>${fn:substring(obj.zxnr,0,110)}</span><br />
										<c:if test='${fn:contains(operationlist,"查看") }'>
											<a href="javascript:show(${obj.id},true);">【查看详情】</a>
										</c:if>
									</div>
							...
							<tbody>
								<c:forEach items="${_page.data }" var="obj" varStatus="i">
									<tr <c:if test="${i.count%2 == 0 }">class="ancolorbg"</c:if>>
										...
										<td>${obj.yhname}</td>
										<td>${obj.flname}</td>
										<td><fmt:formatDate value="${obj.zxsj }"
