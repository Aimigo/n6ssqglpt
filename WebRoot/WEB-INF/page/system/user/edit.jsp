...
<script type="text/javascript">
	$(function(){
		$("#update").click(function(){
			...
			var oldusername = form.oldusername.value;
		...
		$("input[name='rolecode']").click(function(){
			...
			var grid1 = '<td>所属网格（<font class="fred">*</font>）</td>'
						...
						+'		<option value="${grid.name}" <c:if test="${fn:trim(tblUser.gridname) eq fn:trim(grid.name)}">selected="selected"</c:if>>${grid.name}</option>'
								</c:forEach>
						+'	</select>'
						+'</td>';
			var grid2 = '<td>管理网格（<font class="fred">*</font>）</td>'
						...
						+'		<c:if test="${fn:contains(tblUser.gridname ,fn:trim(grid.name)) }"> checked="checked" </c:if>'
		...
		$("input[name='rolecode'][value='${urole.code}']").trigger('click');
		$("#tr_department select").val("${tblUser.departmentid}");
		$("#tr_health select").val("${tblUser.healthid}");
		function qxxz(){
			$('input[type="checkbox"][name="gridname"]:checked').each(function() {
...
<body onload="MM_preloadImages('fun/system/images/nchjbxx-nav01-.png')">${fn:trim(tblUser.gridname)}
	...
	<div class="nchjbxx-mainbg box">
		...
		<div class="nchjbxx-mainbgright">
			<div class="nchjbxx-main">
				...
				<div class="sitebg">
					<div class="site">
						<b>当前位置：共青团农场社区管理平台&nbsp;&gt;&gt;&nbsp;系统管理&nbsp;&gt;&gt;&nbsp;用户管理&nbsp;&gt;&gt;&nbsp;修改</b>
					</div>
				</div>
				<!--当前位置  结束-->
				
				<!--表单内容  开始-->
				<form action="user_update.action" method="post" name="updateform">
				...
				<div class="biaodan-all" style="height:800px;">
					...
					<table class="biaodan" cellpadding="0" cellspacing="0">
						...
						<tr>
							...
							<td colspan="3">
								<input name="username" type="text" class="qbg" maxlength="25" value="${tblUser.username }"/>
							</td>
						</tr>
						<tr>
							<td>密码（<font class="fred">*</font>）</td>
							<td colspan="3">
								<input name="password" type="text" class="qbg" maxlength="25" value="${tblUser.password }"/>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3">
								<input name="realname" type="text" class="qbg" maxlength="25" value="${tblUser.realname }"/>
							</td>
						</tr>
						<tr>
							<td>出生日期</td>
							<td>
								<input name="age" type="text" class="qbg" maxlength="25" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value='<fmt:formatDate value="${tblUser.age }" pattern="yyyy-MM-dd" />'/>
							</td>
							<td>性别</td>
							<td>
								<input type="radio" name="sex" class="input" value="男" <c:if test="${tblUser.sex eq '男' }">checked="checked"</c:if>/>&nbsp;<label>男</label>
								<input type="radio" name="sex" class="input" value="女" <c:if test="${tblUser.sex eq '女' }">checked="checked"</c:if>/>&nbsp;<label>女</label>
							</td>
						</tr>
						<tr>
							<td>邮箱</td>
							<td>
								<input name="email" type="text" class="qbg" maxlength="40" value="${tblUser.email}"/>
							</td>
							<td>电话（<font class="fred">*</font>）</td>
							<td>
								<input name="phone" type="text" class="qbg" maxlength="25" value="${tblUser.phone}"/>
							</td>
						</tr>
						<tr>
							<td>地址</td>
							<td colspan="3"><input name="address" type="text" class="qbg" maxlength="40" value="${tblUser.address }"/></td>
						</tr>
						<tr>
							<td>描述</td>
							<td colspan="3"><input name="ms" type="text" class="qbg" maxlength="250" value="${tblUser.ms }"/></td>
						</tr>
						<tr>
							<td>是否禁用（<font class="fred">*</font>）</td>
							<td colspan="3">
								<input type="radio" name="state" class="input" value="启用" <c:if test="${tblUser.state eq '启用' }">checked="checked"</c:if>/>&nbsp;<label>启用</label>
								<input type="radio" name="state" class="input" value="禁用" <c:if test="${tblUser.state eq '禁用' }">checked="checked"</c:if>/>&nbsp;<label>禁用</label>
							</td>
						</tr>
						<tr>
						...
						<tr id="tr_grid2" style="display:none;">
