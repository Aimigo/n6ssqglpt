...
<script type="text/javascript">
	...
	function deleteIt(id) {
		if (confirm("您确定要删除吗？")) {
			$.ajax({
				type : "post",
				url : "loginlog_delete.action",
				data : {
					"id" : id
				},
				dataType : "html",
				success : function(data) {
					window.location.href = "loginlog_list.action";
				}
			});
		}
	...
	/*展示查看窗体*/
	function show(id) {
		var url = "loginlog_detail.action?id="+id;
		location.href = url;
	}
	...
	<div class="nchjbxx-mainbg box">
		...
		<div class="nchjbxx-mainbgright">
			<div class="nchjbxx-main">
				...
				<div class="sitebg">
					<div class="site">
						<b>当前位置：共青团农场社区管理平台&nbsp;&gt;&gt;&nbsp;系统管理&nbsp;&gt;&gt;&nbsp;日志管理&nbsp;&gt;&gt;&nbsp;登录日志管理</b>
					</div>
				</div>
				<!--当前位置  结束-->
				...
				<div class="tdzy-tab-down">
					<!--查询  开始-->
					<form action="loginlog_list.action" method="post">
						...
						<span>
							<label>
								<select name="cond" class="bd-r" onchange="shownrspan(this.value);">
							        ...
							        <option value="2" <c:if test="${_page.params.cond==2}">selected="selected"</c:if>>真实姓名</option>
							        <option value="3" <c:if test="${_page.params.cond==3}">selected="selected"</c:if>>登录时间</option>
							    </select>
							</label>
					...
					<div class="tab-border">
						<table class="zhwxx-tab">
							...
							<tbody>
								<c:forEach items="${_page.data }" var="loginlog" varStatus="i">
									<tr <c:if test="${i.count%2 == 0 }">class="ancolorbg"</c:if>>
										<td>
											<input name="check" type="checkbox" value="${loginlog.id }" />
										</td>
										<td>${loginlog.username }</td>
								        <td>${loginlog.realname }</td>
								        <td><fmt:formatDate value="${loginlog.logintime }" type="both"/></td>
								        <td>${loginlog.loginip }</td>
										<td>
											<%--<c:if test='${fn:contains(operationlist,"查看") }'>--%>
												<a href="javascript:show(${loginlog.id});">查看详情</a>&nbsp;
											<%--</c:if>--%>
											<%--<c:if test='${fn:contains(operationlist,"删除") }'>--%>
												<a href="javascript:deleteIt(${loginlog.id});">删除</a>&nbsp;
											<%--</c:if>--%>
										</td>
									</tr>
								</c:forEach>
